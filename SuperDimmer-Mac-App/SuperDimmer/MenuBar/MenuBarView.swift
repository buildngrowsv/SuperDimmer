                ...
                VStack(spacing: 8) {
                    ...
                    if settings.intelligentDimmingEnabled {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Picker("", selection: $settings.detectionMode) {
                            ...
                            Text(settings.detectionMode.description)
                                .font(.caption2)
                                .foregroundColor(.secondary)
                        }
                        .padding(.vertical, 4)
                        
                        // Show grid size slider only for per-region mode
                        if settings.detectionMode == .perRegion {
                            VStack(alignment: .leading, spacing: 4) {
                                HStack {
                                    ...
                                    Text("\(settings.regionGridSize)×\(settings.regionGridSize)")
                                        .font(.caption)
                                        .foregroundColor(.secondary)
                                        .monospacedDigit()
                                }
                                ...
                                Slider(
                                    value: Binding(
                                        get: { Double(settings.regionGridSize) },
                                        set: { settings.regionGridSize = Int($0) }
                                    ),
                                    in: 4...16,
                                    step: 1
                                )
                                ...
                                Text("Higher = more precise but more overlays")
                                    .font(.caption2)
                                    .foregroundColor(.secondary)
                            }
                        }
